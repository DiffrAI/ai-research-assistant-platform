    ...
    try:
        ...
        if not customer_id:
            return create_response(
                data=None, message="Failed to create customer", status_code=400
            )

        # Create checkout session
        checkout_url = await payment_service.create_checkout_session(
            customer_id, request.plan, request.success_url, request.cancel_url
        )

        if not checkout_url:
            return create_response(
                data=None, message="Failed to create checkout session", status_code=400
            )
...
async def create_portal_session(
    request: PortalRequest, current_user: User = Depends(get_current_user)
) -> dict:
    ...
    try:
        ...
        portal_url = await payment_service.create_portal_session(
            customer_id, request.return_url
        )

        if not portal_url:
            return create_response(
                data=None, message="Failed to create portal session", status_code=400
            )
...
async def get_subscription_plans() -> dict:
    ...
    return create_response(
        data={"plans": plans}, message="Subscription plans retrieved successfully"
    )
