...
async def get_subscription_info(current_user: User = Depends(get_current_user)) -> dict:
...
@router.post("/refresh")
async def refresh_token(current_user: User = Depends(get_current_user)) -> dict:
    ...
    return create_response(
        data={"access_token": access_token, "token_type": "bearer", "expires_in": 1800},
