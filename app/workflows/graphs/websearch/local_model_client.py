...
class LocalModelClient:
    ...
    def __init__(self, max_retries: int = 3, retry_delay: float = 1.0):
        ...
        self.client = ChatOpenAI(
            ...
            max_tokens=settings.local_model.max_tokens,
    ...
    def _is_retryable_error(self, error: Exception) -> bool:
        ...
        retryable_patterns = [
            ...
            "service unavailable", "gateway", "bad gateway", 
    ...
    def invoke(self, messages: List[BaseMessage]) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                logger.debug(f"Invoking local model (attempt {attempt + 1}/{self.max_retries})")
                ...
                last_error = e
                logger.warning(f"Local model invocation error (attempt {attempt + 1}): {e}")
                ...
                if attempt == self.max_retries - 1:
                    logger.error(f"All {self.max_retries} retries failed for local model")
                    break
    ...
    def invoke_with_structured_output(self, messages: List[BaseMessage], schema: TypeVar('T')) -> T:
    ...
    def _parse_structured_response(self, content: str, schema: TypeVar('T')) -> T:
    ...
    def _extract_key_value_pairs(self, content: str, schema: TypeVar('T')) -> Dict[str, Any]:
    ...
    def _extract_from_markdown(self, content: str, schema: TypeVar('T')) -> Dict[str, Any]:
    ...
    def _infer_from_content(self, content: str, schema: TypeVar('T')) -> Dict[str, Any]:
        ...
        for field in schema_fields:
            ...
            if field.lower() in content_lower:
                ...
                value_match = re.search(rf'{field.lower()}[:\s]+([^\n,;]+)', context, re.IGNORECASE)
                if value_match:
