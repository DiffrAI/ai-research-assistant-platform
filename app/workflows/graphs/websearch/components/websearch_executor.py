...
class WebSearchExecutor:
    ...
    def search(self, state: AgentState) -> dict[str, list[dict[str, Any]]]:
        ...
        # Final fallback to original question
            ...
            "question" in state
            and state["question"]
            and hasattr(state["question"], "content")
        ):
                ...
                try:
                    ...
                    if isinstance(search_response, dict):
                        if "error" in search_response:
                            logger.warning(
                                f"Search tool returned error for '{query}': {search_response['error']}"
                            )
                        ...
                        search_results = (
                            search_response if isinstance(search_response, list) else []
                        )
                    ...
                    logger.info(
                        f"Successfully processed {len(query_results)} results for query: '{query}'"
                    )
                    ...
                    error_msg = str(e)
                    logger.warning(
                        f"Error during web search for query '{query}' (attempt {attempt + 1}): {error_msg}"
                    )

                    if attempt == self._max_retries - 1:
                        logger.error(
                            f"All retries failed for query '{query}': {error_msg}"
                        )
                        failed_queries.append(query)
                    continue
        ...
        logger.info(
            f"Web search completed: {successful_queries}/{total_queries} queries successful, {len(results)} total results"
        )
